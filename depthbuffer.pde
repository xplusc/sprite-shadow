...
class DepthBuffer {
  ...
  void clear(Camera c)
  {
    min = distanceFromCameraPlane(gridToWorld(screenToGrid(new PVector(0, c.tl.y + c.h - 1)))); // x-value doesn't matter
    max = distanceFromCameraPlane(gridToWorld(screenToGrid(new PVector(0, c.tl.y))));
    for (int y = 0; y < h; ++y) {
      for (int x = 0; x < w; ++x) {
